...
#################################################################################
async def _get_current_user(token: str, expected_token_type: str = ACCESS_TOKEN_TYPE):
    ...
    if session_version is None or session_version != user.session_version:
        raise credentials_exception
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    return await _get_current_user(token, ACCESS_TOKEN_TYPE)
...
class OAuth2PasswordBearerWithCookie(OAuth2):
    ...
    async def __call__(self, request: Request) -> Optional[str]:
        authorization = request.cookies.get("refresh_token") or request.cookies.get("Authorization")
        scheme, param = get_authorization_scheme_param(authorization)
        if not authorization or scheme.lower() != "bearer":
            if authorization and request.cookies.get("refresh_token"):
...
async def get_current_user_from_cookie(
    ...
    return await _get_current_user(token, REFRESH_TOKEN_TYPE)


async def get_current_user_from_otp_cookie(request: Request):
    token = request.cookies.get("otp_login_token")
    if not token:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="OTP challenge is not active",
            headers={"WWW-Authenticate": "Bearer"},
        )
    return await _get_current_user(token, OTP_TOKEN_TYPE)


async def get_optional_current_user(token: Optional[str] = Depends(OAuth2PasswordBearer(
    tokenUrl=f"{settings.API_VERSION_STR}/login/access-token",
    auto_error=False,
))):
    if not token:
...
optional_oauth2_scheme_with_cookies = OAuth2PasswordBearerWithCookie(
    tokenUrl=f"{settings.API_VERSION_STR}/login/refresh-token",
    auto_error=False,
)
...
async def get_optional_current_user_from_cookie(
    token: Optional[str] = Depends(optional_oauth2_scheme_with_cookies),
):
    if not token:
        return None
    try:
